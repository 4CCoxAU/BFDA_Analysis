...
sim.H1 <- BFDA.sim(expected.ES=rnorm(1000, 0.7301, 0.36185), type="t.paired",
                   prior=list("Cauchy",list(prior.location=0, prior.scale=sqrt(2)/2)),
                   n.min=20, n.max=80, design = c("sequential"), boundary=10, alternative=c("greater"), B=1000,
                   verbose=TRUE, cores=4, stepsize = 10)

#Simulate null hypothesis
sim.H0 <- BFDA.sim(expected.ES=0, type="t.paired",
                   prior=list("Cauchy", list(prior.location=0, prior.scale=sqrt(2)/2)),
                   n.min=20, n.max=100, design = c("sequential"), boundary=10, alternative=c("greater"), B=1000,
                   verbose=TRUE, cores=4, stepsize = 10)
...
plot(sim.H1, n.min=20, boundary=c(1/10, 10), n.trajectories = 200)
